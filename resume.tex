...
\begin{longtable}{p{0.12\textwidth} | p{0.80\textwidth}}
    ...
    \textit{Jul 2020\newline to \newline Present}
    &
    \textbf{Full Stack Developer at LLW Ingeniería S.R.L.}
    \vspace{0.75em} \newline
    Designed and developed a provisioning and management system for an existing \textit{Softswitch} platform using the \textit{Laravel framework}. The system features a two-tiered API architecture: a low-level API offering standard \textit{CRUD} operations, and a high-level API tailored to the client's most common use cases. An administrative web panel, built on top of the low-level API, complements the system by providing intuitive management capabilities. \\
    &
    Designed and developed a system for the telecommunications sector. The system consists of a core application/service developed with \textit{\CS} (\textit{.Net Framework} and \textit{The Mono Project}) for a \textit{Linux} environment, aimed at controlling and managing telephony-related hardware devices to provide voice communication between subscribers lying in two different technologies (\textit{X25/LAPB} and \textit{SIP}). This system is complemented by an administrative web panel, developed using the \textit{Laravel framework}, which allows for provisioning, monitoring, configuration, and overall system management. \\
    &
    Additionally, I have been responsible for maintaining and enhancing a \textit{Softswitch} system developed in \textit{C/\CC} technologies, as well as developing \textit{Python} scripts to automate various tasks across the company for different purposes. \\
